...
async function probeCollections() {
  const collections = ['reviews', 'ratings', 'notifications', 'comments', 'feedbacks', 'driver_ratings', 'trip_ratings'];
  
  for (const name of collections) {
    try {
      const q = query(collection(db, name), limit(1));
      const snap = await getDocs(q);
      console.log(`Collection '${name}': ${snap.empty ? 'Empty' : 'Has data'} (${snap.size} docs sampled)`);
      if (!snap.empty) {
        console.log(`Sample data from '${name}':`, JSON.stringify(snap.docs[0].data(), null, 2));
      }
    } catch (e) {
      console.log(`Collection '${name}': Error or Missing (${e.message})`);
    }
  }
}
