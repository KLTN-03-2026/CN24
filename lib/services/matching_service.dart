...
class MatchingService {
  ...
  Future<void> findAndMatchDriver(RideRequestModel request) async {
    try {
      ...
      print(
        'DEBUG: [MatchingService] Đang tìm kiếm tài xế gần nhất cho chuyến xe: ${request.id}',
      );
      ...
      if (usersSnapshot.docs.isEmpty) {
        print(
          'DEBUG: [MatchingService] Không tìm thấy tài xế nào online trong bảng users.',
        );
      ...
      print(
        'DEBUG: [MatchingService] Tài xế THỰC SỰ online (từ bảng users): $onlineDriverIds',
      );
      ...
      for (var driverId in onlineDriverIds) {
        ...
        if (locDoc.exists) {
          ...
          driversWithDistance.add({'driverId': driverId, 'distance': dist});
        }
      }

      if (driversWithDistance.isEmpty) {
        print(
          'DEBUG: [MatchingService] Không tìm thấy vị trí cho bất kỳ tài xế online nào.',
        );
        await _rideRepository.updateRideStatus(request.id, RideStatus.timeout);
      ...
      print(
        'DEBUG: [MatchingService] Tìm thấy ${driversWithDistance.length} tài xế đang hoạt động.',
      );
      print(
        'DEBUG: [MatchingService] Danh sách tài xế ưu tiên: ${driversWithDistance.map((d) => "${d['driverId']} (${d['distance'].toStringAsFixed(2)}km)").toList()}',
      );
      ...
      for (var driver in driversWithDistance) {
        ...
        if (currentRequest == null ||
            currentRequest.status == RideStatus.cancelled) {
          print(
            'DEBUG: [MatchingService] Dừng tìm kiếm vì chuyến đi đã bị hủy hoặc không tồn tại.',
          );
          return;
        }
        ...
        print(
          'DEBUG: [MatchingService] Đang kiểm tra trạng thái và mời tài xế: $driverId...',
        );
        bool result = await _tryAssignDriver(
          request.id,
          driverId,
          driver['distance'],
        );
        if (result) {
          print(
            'DEBUG: [MatchingService] CHÚC MỪNG: Tài xế ${driver['driverId']} đã nhận chuyến.',
          );
          return;
        } else {
          print(
            'DEBUG: [MatchingService] Tài xế ${driver['driverId']} không phản hồi hoặc đã từ chối.',
          );
        }
      }
  ...
  Future<bool> _tryAssignDriver(
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        print(
          'DEBUG: [MatchingService] Kiểm tra thực tế tài xế $driverId: isOnline=$isOnlineInDb, isAvailable=$isAvailableInDb',
        );

        if (!isOnlineInDb || !isAvailableInDb) {
          print(
            'DEBUG: [MatchingService] BỎ QUA tài xế $driverId vì trạng thái thực tế là Offline/Bận.',
          );
        ...
        print(
          'DEBUG: [MatchingService] Tài xế $driverId không tồn tại trong users. Xoá khỏi driver_locations và bỏ qua.',
        );
    ...
    print(
      'DEBUG: [MatchingService] Đang thông báo cho tài xế: ${driverName ?? driverId} (Khoảng cách: ${distance.toStringAsFixed(2)} km)',
    );
    await _rideRepository.assignDriver(
      requestId,
      driverId,
      distance,
      driverPhone,
    ...
    print(
      'DEBUG: [MatchingService] Đã gọi assignDriver cho requestId: $requestId',
    );
    ...
    subscription = _rideRepository.watchRideRequest(requestId).listen((
      updatedRequest,
    ) {
      if (updatedRequest.status == RideStatus.accepted) {
        print(
          'DEBUG: [MatchingService] Tài xế $driverId đã trả lời: CHẤP NHẬN',
        );
        ...
        print(
          'DEBUG: [MatchingService] Khách hàng đã hủy chuyến xe. Dừng chờ tài xế.',
        );
        if (!responseCompleter.isCompleted) responseCompleter.complete(false);
        subscription?.cancel();
      }
    ...
    try {
      bool accepted = await responseCompleter.future.timeout(
        ...
        onTimeout: () {
          print(
            'DEBUG: [MatchingService] Tài xế $driverId: HẾT THỜI GIAN phản hồi (Timeout)',
          );
          subscription?.cancel();
          return false;
        },
